...
/*
 * Part 1
 *      Find the number of elements in group 0
 *      Answer = 239
 */

let root = 0;
let input = [];
let groupZero  = new Set();

let prepInput = function prepInput() {
    raw.forEach((val, index) => {
        let row = val.split(" ");
        let key  = Number(row[0]);

        // Add a new array to input to store the parent and all children
        input[index] = [];
        input[index][0] = key;

        // Disregard the first two elements (parent, arrow separator)
        // Remove commas if necessary
        for (let i = 2; i < row.length; i++) {
            input[index][i-1] = row[i].endsWith(",") ? Number(row[i].slice(0, row[i].length-1)) : Number(row[i]);
        }
    });
};

let findGroupZero = function findGroupZero (key) {
    if (groupZero.has(key)) return;
...
let checkMembership = function checkMembership (key) {
    let found = false;
    let index = 0;
    while (!found && index < groups.length) {
        found = groups[index].has(key);
        index++;
    }
    return found;
};

let assignGroup = function assignGroup (key, index) {
    if (groups[index].has(key)) return;
...
let run = function run () {
    ...
    input.forEach((val) => {
        // Check if a program ID is already assigned to a group.  If so, skip.
        if (!checkMembership(val[0])) {
            // Program ID is not already assigned.  Create a new group.
            groups[++currentIndex] = new Set();
            assignGroup(val[0], currentIndex);
        }
    });
};
