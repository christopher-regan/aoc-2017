...
operators.set("inc", "{0}+({1})");
operators.set("dec", "{0}-({1})");

let process = function process (value, operator) {
    let template = operators.get(operator);
    value.forEach((val, index) => template = template.replace(`{${index}}`, val));
    return eval(template);
};

let getRegisterValue = function getRegisterValue (key) {
    return registers.has(key) ? registers.get(key) : 0;
};

let executeInstructions = function findMaxRegisterValue () {
    raw.forEach(val => {
        /* instruciton format
         *  [0] = register
         *  [1] = operator
         ...
         *  [6] = conditional amount
         */
        let i  = val.split(" ");

        if (eval(`${getRegisterValue(i[4])} ${i[5]} ${i[6]}`)) {
            registers.set(i[0], process([getRegisterValue(i[0]), i[2]], i[1]));
            interimMax = getRegisterValue(i[0]) > interimMax ? getRegisterValue(i[0]) : interimMax;
        }
    });
};
...
/*
 * Part 1
 *      Find the max register value after all instructions have been processed
 *      Answer = 4163
 */
...
exports.star1 = function star1 () {
    ...
    executeInstructions();
    registers.forEach((v, k) => max = (v > max) ? v : max);
...
/*
 * Part 2
 *      Find the highest register value ever set during instruction execution
 *      Answer = 5347
 */
