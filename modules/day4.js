...
let hasDuplicates = function (input) {
    return input.some((word, index, array) => array.indexOf(word) !== index);
};

/*  
 *   Part 1
 *      Find the count of passphrases that do not contain any duplicate words.
 *      Answer = 383
 */
let validateNoDuplicates = function (total, val, index, arr) {
    let valid = 0;
    if (!hasDuplicates(val.split(" "))) {
        valid = 1;
    }
    return total + valid;
};
...
exports.star1 = function star1 () {
    return input.reduce(validateNoDuplicates, initialValue);
};

/*  
 *   Part 2
 *      Find the count of passphrases that do not contain any anagrams.
 *      Answer = 265
 */
let validateNoAnagrams = function (total, val, index) {
    let valid = 0;
    // Raw duplicates also qualify as anagrams so count as invalid and skip further checks.
    if (!hasDuplicates(val.split(" "))) {
        // Create an array of re-ordered words, then check again for duplicates.
        let words = val.split(" ")
            .map(s => s.split("")
            .sort()
            .join(""));
...
exports.star2 = function star2 () {
    return input.reduce(validateNoAnagrams, initialValue);
};