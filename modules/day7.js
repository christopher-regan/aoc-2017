...
let prepInput = function prepInput() {
    raw.forEach((val, index) => {
        let row = val.split(" ");
        let key  = row[0];

        // Weight is wrapped in parentheses
        let weight = Number(row[1].slice(1,row[1].length-1));
        weights.set(key, weight);

        // Add a new array to input to store the parent and all children
        input[index] = [];
        input[index][0] = key;

        // Disregard the first three elements (parent, weight, arrow separator)
        // Remove commas if necessary
        for (let i = 3; i < row.length; i++) {
            input[index][i-2] = row[i].endsWith(",") ? row[i].slice(0, row[i].length-1) : row[i];
        }
    });
};

let findRoot = function findRoot() {
    // Get all rows that have children elements
    const parents = input.filter(row => row.length > 1);
    let count = 0;

    while (count < parents.length) {
        let foundParent = false;
        // Check each value in the raw input for the parent identifier specified as a child of any other node
        raw.forEach((val) => {
            if (val.match(new RegExp('->.*' + parents[count][0], 'gi'))) {
                foundParent = true;
            }
        });
        // No parent was found for the current element, so it must be the root
        if (!foundParent) {
            root = parents[count][0];
            break;
        }
        count++;
    }
...
/*
 * Part 2
 *      Find the one imbalanced tower and determine what the weight should be for an even distribution.
 *      Answer = 1505
 */
let calculateTowerWeight = function calculateTowerWeight (key) {
    let weight = 0;

    tower = input.find((tower) => {
        return tower[0] === key; 
    });
    ...
    if (tower.length > 1) {
        ...
        // Disregard the first element, which is the one currently being processed
        tower.slice(1).forEach((sub, index) => {
            children[index] = [];
            children[index][0] = sub;
            children[index][1] = calculateTowerWeight(sub);

            weight += children[index][1];

            if (index > 0) {
                unbalanced = children[index][1] !== children[index-1][1];
            }
        });
...
let run = function run (val) {
    let weight = 0;
    if (val) {
        weight = calculateTowerWeight(val);
    } else {
        weight = input.forEach((element) => {
            calculateTowerWeight(element[0]);
        });
    }
    return weight;
};
